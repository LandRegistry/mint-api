...
class CryptographicSigning(object):
    ...
    def sign_payload(self, payload):
        current_app.logger.info("Sign the payload")
        if not hasattr(self, 'cyptosign_sign'):
            self.cyptosign_sign = self._create_signer()
        digest = SHA256.new()
        digest.update(payload.encode('UTF-8'))
        sign = self.cyptosign_sign.sign(digest)
        current_app.logger.info("Payload signature and hash generated")
        return "rs256:{0}".format(b64encode(sign).decode('UTF-8')), "sha-256:{0}".format(digest.hexdigest())
    ...
    def _create_signer(self):
        current_app.logger.info("Create signer")
        key_data = open(current_app.config['PRIVATE_KEY'], 'rb').read()
        rsakey = RSA.importKey(key_data, current_app.config['PRIVATE_PASSPHRASE'])
        signer = PKCS1_v1_5.new(rsakey)
        return signer
